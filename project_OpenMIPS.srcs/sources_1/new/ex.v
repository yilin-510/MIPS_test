...
module ex(
 ...
 // 异常处理：合并译码阶段传来的异常，并加上本阶段产生的溢出(ovassert)和自陷(trapassert)
...
// --- 计数类指令 (CLZ/CLO) ---
 // 实现查找第一个0或第一个1的位置（前导零计数）									
  ...
  // 优先级：访存阶段写回 > 回写阶段写回 > 寄存器堆原始值
	always @ (*) begin
		if(rst == `RstEnable) begin
			{HI,LO} <= {`ZeroWord,`ZeroWord};
		end else if(mem_whilo_i == `WriteEnable) begin
			{HI,LO} <= {mem_hi_i,mem_lo_i}; // 访存阶段前推
		end else if(wb_whilo_i == `WriteEnable) begin
			{HI,LO} <= {wb_hi_i,wb_lo_i}; // 回写阶段前推
		end else begin
			{HI,LO} <= {hi_i,lo_i};	// 正常读取		
		end
	end	
  ...
  // --- 乘累加逻辑 (MADD/MSUB) ---
  // 第一周期计算乘法并暂停，第二周期加到 HI/LO 上并停止暂停
	always @ (*) begin
		if(rst == `RstEnable) begin
